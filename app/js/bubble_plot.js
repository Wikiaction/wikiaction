...
for (i = 0; i < project_data.length; i++) {
	if (Math.sqrt(project_data[i]["seuil_unit_engagement"]) > node_max_seuil){
		node_max_seuil = Math.sqrt(project_data[i]["seuil_unit_engagement"]);
	}
}
...
for (i = 0; i < project_data.length; i++) {
	nodes.push({
		...
		size: max_node_size * Math.sqrt(project_data[i]["seuil_unit_engagement"])/node_max_seuil,
...
vis.selectAll("path").data(nodes).on('click' , function(d){ console.log(d.title); })
...
// Old CODE GAEL
// var tltips = d3.select("body").select("svg").append("g").append("g").attr("class", "tooltip").append("div").html("test");
// // var tltips = d3.select("body").select("svg").prepend("div").attr("class", "tooltip");
...
// 	tooltip.select("div").html(d.title);
...
oReq.open("get", "get_data_bubble_plot.php", true);
oReq.send();
